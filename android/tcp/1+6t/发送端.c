...
// 分辨率配置（从命令行读取或使用默认值）
int sender_width = 1440;
int sender_height = 3168;
int receiver_width = 1080;
int receiver_height = 2376;
    ...
    while (1) {
        ...
        if (ev.type == EV_ABS) {
            if (ev.code == ABS_MT_POSITION_X || ev.code == ABS_X) {
                send_ev.value = 1080- map_x(ev.value);
		send_ev.code=ABS_MT_POSITION_Y; 
                printf("[%d] X: %d -> %d\n", ++event_count, ev.value, send_ev.value);
            } 
            else if (ev.code == ABS_MT_POSITION_Y || ev.code == ABS_Y) {
                send_ev.value =  map_y(ev.value);
		send_ev.code=ABS_MT_POSITION_X;
                printf("[%d] Y: %d -> %d\n", event_count, ev.value, send_ev.value);
            }
        }
